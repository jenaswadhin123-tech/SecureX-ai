  ...
  const headerToggle = (
    <div className="flex items-center mb-2">
      <input
        ...
        type="checkbox"
        checked={headerMode}
        onChange={() => setHeaderMode(!headerMode)}
        className="w-4 h-4 text-accentBlue bg-darkBg border-gray-300 rounded focus:ring-accentBlue mr-2"
      />
      <label htmlFor="headerMode" className="text-sm text-secondaryText select-none">
        Raw Email Header Mode
      </label>
    </div>
  );

  // Adjust placeholder based on header mode
  const placeholderText = headerMode
    ? "Paste raw email headers here..."
    : "Paste email content or suspicious SMS text here...";
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-cardBg border border-cardBorder p-6 rounded-xl">
        <div className="flex items-center space-x-3 mb-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Phishing Text & Email Analyzer</h3>
            <p className="text-xs text-secondaryText">Detects social engineering, urgency cues, and credential solicitation patterns.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleScan} className="mt-5 space-y-4">
          ...
          <textarea
            ...
            className="w-full bg-darkBg border border-cardBorder focus:border-accentBlue rounded-xl p-4 text-sm text-white focus:outline-none transition-colors font-sans"
          />
          ...
          >
            ...
            <span>{loading ? 'Analyzing Content...' : 'Analyze Phishing Traits'}</span>
          </button>
        </form>
      ...
      {result && (
        <div className="bg-cardBg border border-cardBorder p-6 rounded-xl space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-cardBorder">
            <div>
              <span className="text-xs font-mono text-secondaryText uppercase tracking-wider">Phishing Risk Assessment</span>
          ...
          {result.evidence && result.evidence.length > 0 ? (
            <div>
              <h5 className="text-xs font-semibold text-secondaryText uppercase tracking-wider mb-2">Social Engineering Indicators</h5>
            ...
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs rounded-lg flex items-center gap-2">
              ...
              <span>No overt phishing keywords or social engineering triggers identified.</span>
            </div>
          )}
          ...
          {result.recommendations && result.recommendations.length > 0 && (
            <div>
              <h5 className="text-xs font-semibold text-secondaryText uppercase tracking-wider mb-2">Recommendations</h5>
